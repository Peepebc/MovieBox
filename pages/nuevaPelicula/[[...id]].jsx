...
export default function NuevaPelicula() {
  ...
  async function onSubmit(data) {
    ...
    formData.append(
      "Director",
      data.Director ? data.Director : pelicula.director
    );
    ...
    formData.append(
      "Descripcion",
      data.Descripcion ? data.Descripcion : pelicula.descripcion
    );
    if (id) {
    ...
    <>
      <div className="flex md:justify-center items-center ">
        <div className=" md:flex flex-row shadow-2xl shadow-black rounded-lg">
          <div
            className="rounded-lg p-10  h-auto"
            style={{ backgroundColor: "#1F2E55" }}
          >
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="flex flex-col py-3">
                ...
                <input
                  {...register("Titulo")}
                  defaultValue={id && pelicula.titulo}
                  className="focus:outline-none rounded-lg border border-gray-100 bg-transparent p-2"
                  type="text"
                />
              </div>
              <div className="flex flex-col py-3">
                ...
                <input
                  {...register("Director")}
                  defaultValue={id && pelicula.director}
                  className="focus:outline-none rounded-lg border border-gray-100 bg-transparent p-2"
                  type="text"
                />
              </div>
              <div className="flex flex-col py-3">
                ...
                <input
                  {...register("Generos")}
                  defaultValue={id && pelicula.generos}
                  className="focus:outline-none rounded-lg border border-gray-100 bg-transparent p-2"
                  type="text"
                />
              </div>
              <div className="flex flex-col py-3">
                ...
                <input
                  {...register("Fecha")}
                  defaultValue={
                    id && pelicula.fecha && pelicula.fecha.split("T")[0]
                  }
                  className="focus:outline-none rounded-lg border border-gray-100 bg-transparent p-2"
                  type="date"
                />
              ...
              <div className="flex flex-col py-3">
                ...
                <textarea
                  ...
                  defaultValue={id && pelicula.descripcion}
                  className="focus:outline-none rounded-lg border border-gray-100 bg-transparent p-2"
                ></textarea>
              </div>
              {id ? null : (
                <div className="flex flex-row py-3 gap-3 items-center">
                  {urlImagen && (
                    <img className="w-20 h-32 object-contain" src={urlImagen} />
                  )}
              ...
              <div className="flex justify-center py-4">
                <button
                  type="submit"
                  className="rounded-lg p-2 bg-gray-100 text-black"
                >
                  {id ? "EDITAR PELICULA" : "AÑADIR PELICULA"}
                </button>
              </div>
